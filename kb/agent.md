...
Il metodo è nato descrivendo due agenti — l'umano e un singolo LLM — e su quel
binomio ha costruito la sua asimmetria: per l'umano la KB è la vista di un
modello che già possiede, per l'LLM è il modello mentale intero, ricostruito a
ogni sessione. Il binomio è un caso particolare, non la struttura. Norman
definisce l'agente come «person, animal, or machine»: la categoria macchina non
è un punto ma un gradiente, perché gli agenti-macchina differiscono per
capacità, costo, località e fiducia. Un LLM di frontiera che pianifica ad alto
livello e un agente locale più semplice che esegue non sono lo stesso lettore,
anche quando leggono la stessa KB.
...
lavoro per un agente basso. Questo nodo resta in bozza di proposito: apre la
