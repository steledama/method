...
Esiste **dove il dominio ha un territorio da indicizzare** (host, conti, sistemi, entità). Dove il dominio è astratto non serve: in `metodo` non c'è `map.md`, perché il «mondo» sono i nodi stessi e il modello vive nei nodi e in `presentations/`.
