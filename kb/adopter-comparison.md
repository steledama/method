...
La formalizzazione vive nei nodi output (o1 macchina, o2 decisione umana, o3 prescrizione versionata; + strato input i1/i2/i3), world (membrana fisica) e action-cycle (fondamento teorico via Norman). Norman è l'interfaccia di azione dentro la cornice di augmentation che contiene i riferimenti del metodo (cfr. `augmentation-system`), accanto a Luhmann (atomicità) e alla gamba di manutenzione di cui Karpathy è l'istanza contemporanea.
...
- **metodo / skill** — azione: mantenere il quartetto `kb-review` / `plan-review` / `verdicts-review` / `commit` come base ufficiale, lasciando locali le parametrizzazioni di dominio; tipo: regola stabile
