...
La revisione del plan va fatta come controllo leggero a inizio sessione e come controllo completo quando cambiano fatti, scadenze o dipendenze. Questa funzione è codificata nella skill base `plan-review` (già `tasks-review`), che deve verificare drift `plan`/`tasks`, task superati, nuovi task emersi dai fatti, priorità aggiornate, la direzione task→obiettivo verso il register `goal.md` e il task consigliato per la sessione corrente.
