...
La revisione del plan va fatta come controllo leggero a inizio sessione e come controllo completo quando cambiano fatti, scadenze o dipendenze. Questa funzione è codificata nello scope `plan` della skill base `exec` (storia dei nomi: `tasks-review` → `plan-review` → `exec plan`, cfr. `skill`), che deve verificare drift `plan`/`tasks`, task superati, nuovi task emersi dai fatti, priorità aggiornate, la direzione task→obiettivo verso il register `goal.md` e il task consigliato per la sessione corrente.
