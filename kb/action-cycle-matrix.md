...
- **Perceive · i1 — S**: `o3/kb_tools.py` via `/kb` cattura il segnale sull'artefatto — link rotti, orfani, copertura del catalogo, audit «45 nodi, 0 link rotti» — valenza-neutro.
...
- **Plan · o1 — D**: il protocollo d'audit periodico top-down esiste ed è dichiarato: la skill di dominio `/adottanti` (il «cosa si audita, come»), la cadenza `(mensile)` in `## Scadenze` di `o1/plan.md`, gli esiti che aggregano nel filo `i3/audit-adottanti.md`. La metrica è la distanza dal telos letta dalla composizione delle code (`development-goal`, punto asintotico) — audit, non micromanagement. D e non S perché l'istituzione ha un solo battito (primo audit 2026-07-11): la ripetibilità è dichiarata, non ancora dimostrata.
...
- **Perform · o3 — S**: `o3/` è il canale o3 reale e canonizzato in `project-structure`; runbook reali vi sono transitati. Oggi senza prescrizioni attive, ma **ha già funzionato**: S per capacità reale, non per pienezza istantanea.
- **Perceive · i1 — S**: `i1/` è il canale i1 reale: i segnali degli adottanti vi si catturano valenza-neutri e, consumato il verdetto, si potano — la collezione tiene solo gli aperti, la storia in git (cfr. `perceive`). Ha già funzionato, dieci segnali catturati e consumati: stesso metro di o3.
- **Interpret · i2 — D**: l'interpretazione del Mondo-runtime è l'**osservatorio** — la rilettura comparativa degli adottanti. Ha la sua **prima istanza** (`i2/baricentro-kb-adottanti.md`, i2-runtime: legge i cataloghi `kb/` dei domini, non i nodi del metodo) più i ledger `method` come materiale grezzo. D perché è una sola lettura, non un osservatorio stabile e periodico. La call A "a caldo" (`metodo-in-sintesi.md` è i2-runtime) è **ribaltata**: la sintesi i2 del metodo interpreta il metodo, non i domini — la deriva non esisteva, era un'assegnazione sbagliata. Segnare S qui sarebbe la simmetria-compiacente che la matrice esiste per prevenire.
- **Compare · i3 — D**: la valutazione-osservatorio — confrontare la fotografia dei domini adottanti contro gli obiettivi-runtime — è inquadrata da `adopter-comparison` (la lente) e vive in parte nei fili adottanti di `i3/`; il filo `i3/audit-adottanti.md` la materializza ora come verdetto-runtime a sé, giovane di un solo battito. Presente, in consolidamento.
...
Conteggio `metodo`, 16 celle: **11 S, 5 D, 0 vuoti, 0 F** (dev: 7 S, 1 D; runtime: 4 S, 4 D). Zero forzati, ma il dato che parla è la _forma_: le cinque D segnano gli archi immaturi — incluso l'**interno runtime**, che la rilettura a freddo ha ricondotto da pieno (la sintesi i2 del metodo) a nascente (l'osservatorio, una sola istanza); l'ex vuoto-gap runtime-o1 è **chiuso a D** dal protocollo `/adottanti` (primo audit 2026-07-11). Il Goal-dev è risalito da D a S una volta articolato in `development-goal`; resta D nel dev solo `metodo-in-sintesi.md` (i2-dev), dove la disciplina di derivazione è il limite.
...
- **Call B — confermata.** L'**arco top-down del runtime** (o1) è il vero buco — non un vuoto-per-design ma un vuoto-gap, negato finora dal principio «`metodo` non orchestra task sui adottanti». L'arco è ora **scoccato end-to-end una volta** (prescrizione baricentro → `method` degli adottanti → recepimento), prova che è percorribile; il **protocollo** o1-runtime — l'audit periodico, non il micromanagement delle code — è poi stato istituito: skill `/adottanti`, cadenza mensile in `## Scadenze`, esiti aggregati nel filo `i3/audit-adottanti.md` (primo audit 2026-07-11). O1 sale da vuoto-gap a D.
...
Le asimmetrie reali, a freddo: **dev-i2 è la sintesi illustrata del metodo** (presente ma a derivazione non disciplinata, D), mentre **l'i2/o2-runtime è l'osservatorio nascente** (una istanza, D); e **runtime-o1 appena istituito** — l'arco top-down ha ora il suo protocollo (`/adottanti`), giovane di un solo battito.

### Verdetto-gate

La verifica a 16 celle **regge per `metodo`** — questo è fermo, e ratificato a freddo: non una cucitura su mezzo ciclo, ma **runtime e dev genuini** (11 S, 5 D, 0 F). Ma il verdetto sano **non è «due cicli pieni»**: è **«runtime genuino ma nascente, con meta-ciclo di sviluppo già più maturo»**. La prima stesura sottostimava il runtime (membrana su interno vuoto); il ribaltamento "a caldo" lo sovrastimava (interno pieno via sintesi i2 del metodo); la rilettura a freddo lo coglie per quello che è: archi reali, interno immaturo.

- **Regge la struttura**: zero celle forzate; entrambi i cicli hanno poli costituiti e membrana fisica (o3/i1 runtime).
- **L'interno runtime è nascente, non pieno**: l'i2/o2-runtime è l'osservatorio a una sola istanza (`baricentro-kb-adottanti.md`), non `metodo-in-sintesi.md` (che è i2-dev); l'i3-runtime comincia a materializzarsi nel filo dell'audit; e **runtime-o1 è appena istituito** — il protocollo `/adottanti` al suo primo battito.
...
Prodotta perché (a) regge. Per ciascuno dei 16 elementi indica il file che i task di presentazione consumano senza inventare la geometria a mano.
...
- i1-dev → `o3/kb_tools.py` (`/kb`: audit, backlink, copertura)
...
- o1-runtime → skill `/adottanti` + riga `(mensile)` in `## Scadenze` di `o1/plan.md`; esiti aggregati nel filo `i3/audit-adottanti.md`
...
- i3-runtime → `i3/audit-adottanti.md` (verdetto aggregato dell'audit) + `adopter-comparison` (lente) + fili adottanti in `i3/` + ledger `method`
