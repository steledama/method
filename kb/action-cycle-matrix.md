...
- **Interpret · i2 — D**: il deck `i2/metodo-in-sintesi.md` è l'auto-interpretazione dello sviluppo — la sintesi illustrata del metodo, dei nodi e dell'architettura dell'artefatto: **i2-dev** (e o2-dev quando lo si usa come vista di decisione sul metodo). La rilettura a freddo del gate ha **ribaltato la riassegnazione "a caldo" al runtime**: il deck rilegge il Mondo-dev (i nodi del metodo), non i quattro domini, quindi è dev, non runtime — e l'indice `i2/interpretations.md` già lo classifica così. D e non S perché la sua derivazione non è disciplinata (è `bozza`, e i deck driftano se non ri-generati: è il presidio di `cognitive-fidelity`).
...
- i2-dev → `i2/metodo-in-sintesi.md` (il deck: sintesi del metodo; o2-dev quando è vista di decisione)
- i3-dev → fili in `i3/`
