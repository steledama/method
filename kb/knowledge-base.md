...
Il sistema poggia su tre pilastri metodologici: lo Zettelkasten, che fornisce la filosofia dei singoli nodi (atomicità, interconnessione, struttura emergente dal basso); il pattern Karpathy, che fornisce la metodologia di manutenzione dell'insieme (ingest, query, lint, filing back); Norman, che fornisce il modello del ciclo di azione tra utente e sistema — come la KB traduce conoscenza in decisione e comportamento reale. I tre si completano: lo Zettelkasten dice come pensare i nodi, il pattern Karpathy dice come gestire il sistema nel tempo con l'aiuto dell'LLM, Norman dice come il sistema chiude il ciclo producendo azione nel mondo.

La KB non è il fine: è strumentale all'azione. Lo strato output — chiamato ponte nel metodo, con nomi locali nei singoli progetti — traduce la conoscenza accumulata in azione possibile e separa tre livelli: output macchina (L1, per LLM e automazione), output decisione (L2, per l'utente che deve decidere), output azione nel mondo (L3, fuori dal repo). Senza questo strato, la KB accumula conoscenza ma non chiude il ciclo tra comprensione e comportamento.
