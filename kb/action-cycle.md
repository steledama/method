...
**Ciò che l'artefatto trattiene sono i prodotti degli atti.** Ogni atto del ciclo non è solo un'operazione che passa: _deposita un prodotto versionato_, in un luogo proprio dell'atrio — o1 in `o1/plan.md`, o2 in `o2/`, o3 in `o3/` e nei commit, i1 in `i1/`, i2 nei nodi `kb/` e in `i2/`, i3 nei fili `i3/` (la mappa-sorgente completa è in `action-cycle-matrix`). Anche i poli trattengono stato: il Goal è scritto (`README`, `development-goal`), e il Mondo «remembers by being there». Nessuno stadio del loop è effimero: ciascuno _è_ e _contiene_ il prodotto dell'agire.
