...
**Lo sweep dei quattro host (2026-06-10).** Spenta `auto-memory`, ho ispezionato gli store residui su `svezia`, `deck`, `game` (`neve` rimandato). L'esito conferma la tesi dal basso: di ~15 file di memoria, **uno solo** conteneva qualcosa di nuovo e generale — il principio proposta→consenso, ora nodo [`consenso`], innescato da una feedback di `salute`. Tutto il resto era già nei repo versionati e in forma più completa (il gotcha Baserow in `bi/kb/baserow-formule.md`; i fatti fiscali in nodi dedicati come `economia/kb/carla-tacchi.md`; la KB tecnica in `comfyui/CLAUDE.md`), oppure storico/superato, oppure — caso `comfyui`, dove la memoria affermava «custom node via symlink» e il `CLAUDE.md` versionato «non usare symlink, Docker non li segue» — in contraddizione col canone, cioè stale. La memoria host-locale si è rivelata in larga parte ridondante, e in un caso pericolosa, rispetto al KB disciplinato: prova concreta che la memoria versionata nel repo non è solo più portabile, è più affidabile.
...
### [2026-06-10] La forma del plan è uniforme su tutti i repo — e l'innesco è `method-review`

Confronto con Stefano su come propagare la forma agli altri tre adottanti. Reazione netta: invariante di Class A, uniforme ovunque — e l'argomento è costituzionale, più affilato della mia carve-out «dove la coda è minima un elenco basta». In questo repo entra solo ciò che è generale (i1); quindi «sta qui» implica «si applica a tutti», e nessuna coda è tanto piccola da giustificare una forma propria. La carve-out faceva entrare di soppiatto una considerazione di dominio (la coda di `nixos`) in un nodo che deve contenere solo l'invariante: ri-derivata la riga come regola dell'atrio calata sul plan (struttura uniforme, carattere nel contenuto). È un i3 sul commit di stamattina (`85927c9`) — un'assunzione che ha cambiato significato e va ri-valutata, non lasciata stale.

Sul _come_: non un runbook in `metodo` che orchestra i figli (lo vieta il nodo stesso), ma una skill dedicata **`method-review`** — già in `tasks/`, qui messa a fuoco — speculare a `audit-kb` («sano dentro?» vs «allineato al canone?»). Candidato per tenerla economica: un watermark per adottante (l'ultimo SHA di `metodo` riconciliato) che riduce «rileggi tutta la kb» a `git log watermark..HEAD`. Disciplina bottom-up dentro lo strumento: rileva e propone sempre, auto-applica solo il meccanico, mette in gate ciò che richiede adattamento locale (`salute` coi task senza file è un parametro sotto-specificato della forma, non un'eccezione). Questa modifica del plan è il primo drift su cui pilotarla. Intanto, primo passo a mano: `economia` (bisogno massimo).

### [2026-06-10] La forma del `plan.md`: identificatori a vite diverse — e perché _non_ self-contained

Da `bi` (commit `6ba766a0`) è emersa una forma migliore del `plan.md`: tabella stretta ordinata per esecuzione, con **due identificatori a vite diverse** — il `#` effimero (ordine, vive solo in tabella, si rinumera lì) e il link al file `tasks/` stabile (footer per titolo, disaccoppiato dall'ordine). Riferire una dipendenza come `#2` invece del titolo tiene stretta la colonna senza duplicare nulla; riordinare le priorità tocca una lista sola, non due. L'identità sta in un posto, l'ordine in un altro, e non si rincorrono: è la fonte-unica calata sulla manutenzione del plan. Questo è il guadagno portabile, e va nel nodo [`plan`].

`bi` aveva anche reso il file **self-contained**, incorporandovi le istruzioni di consultazione e compilazione. Discutendolo con Stefano, l'abbiamo respinto come canone. Tre ragioni convergono. **Fonte-unica:** se la forma vive nel file _e_ nel nodo è duplicata — la stessa ironia già colta il 2026-06-03 (il metodo che predica single-source e lo viola su sé stesso). **Symlink:** il nodo propaga una modifica a tutti gli adottanti con un edit; le istruzioni embedded in ogni `plan.md` no — il self-contained _inverte_ il vantaggio architetturale del repo. **Norman:** la mossa corretta non è aggiungere un signifier che ripete, ma una forma che **affordi da sé** — la tabella letta si auto-spiega — più la convenzione documentata una volta nel nodo, a cui `CLAUDE.md`/`AGENTS.md` _rimandano_ senza ripeterla. È la stessa lezione dell'atrio ([`affordance-signifier`], [`system-image`]): un file che spiega un altro file è il cartello «tirare»; i dettagli stanno nel maestro, non incollati su ogni istanza.
