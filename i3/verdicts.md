...
- [audit-adottanti.md](audit-adottanti.md) — verdetto aggregato dell'audit mensile `/adopters-review` (secondo battito 2026-08-01, anticipato): i quattro `aligned`, scostamento di `economia` sciolto, terza specie maturata in timetable canonico (`bi`), due date stantie in `nixos` (coda di dominio); tutti attendono la quinta domanda e poi la rifilatura — misura: «Propagare il canone e chiudere il loop»
...
- [skill-per-arco-tripartito.md](skill-per-arco-tripartito.md) — le skill si tagliano per arco e non per indice: `plan-review`/`verdicts-review` → `exec`/`eval`, tripartite sui sei scope-stadio; il montaggio delle skill di dominio come argomenti resta ipotesi sperimentale da pilotare via `method-review` nell'ordine `nixos` → `salute` → `economia` → `bi`; primo giro end-to-end dei sei scope compiuto in `metodo` (2026-08-01), gambe deboli e clausola di uscita dichiarate prima di partire — misura: «Custodire un canone coerente»
