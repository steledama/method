...
- [skill-per-arco-tripartito.md](skill-per-arco-tripartito.md) — le skill si tagliano per arco e non per indice: `plan-review`/`verdicts-review` → `exec`/`eval`, tripartite sui sei scope-stadio; il montaggio delle skill di dominio come argomenti resta ipotesi sperimentale da pilotare via `method-review` nell'ordine `nixos` → `salute` → `economia` → `bi`; gambe deboli e clausola di uscita dichiarate prima di partire — misura: «Custodire un canone coerente»
