...
- [skill-per-arco-tripartito.md](skill-per-arco-tripartito.md) — le skill si tagliano per arco e non per indice: `plan-review`/`verdicts-review` → `exec`/`eval`, tripartite sugli stadi del modello con scope invocabile; gli stadi degradati (i1, i2, o3) erano esattamente quelli che nessun nome nominava; movimento dall'alto legittimo, gambe deboli dichiarate (`exec perform` asimmetrico, `eval perceive` a rischio di vuoto) e clausola di uscita scritta prima di partire — misura: «Custodire un canone coerente»
