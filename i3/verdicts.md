...
- [skill-per-arco-tripartito.md](skill-per-arco-tripartito.md) — le skill si tagliano per arco e non per indice: `plan-review`/`verdicts-review` → `exec`/`eval`, tripartite sui sei scope-stadio; ristrutturazione chiusa in `metodo` (2026-08-01: due giri di pilota, canone inciso, prescrizione emessa), restano il recepimento dei quattro e il montaggio di dominio come esperimento sequenziale (`nixos` → `salute` → `economia` → `bi`), con la clausola di uscita in attesa nel task `pause` — misura: «Custodire un canone coerente»
